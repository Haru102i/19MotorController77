...
bsp_int_err_t bsp_interrupt_group_enable_disable (bsp_int_src_t vector, bool enable, uint32_t ipl)
{
    ...
    if      ((vector >= BSP_INT_SRC_BL0_SCI1_TEI1) && (vector <= BSP_INT_SRC_BL0_DOC_DOPCI))
    {
        /* Group BL0. */
        if (true == enable)
        {
    ...
    else if ((vector >= BSP_INT_SRC_BL1_POEG_POEGGAI) && (vector <= BSP_INT_SRC_BL1_SCI9_ERI9))
    {
        /* Group BL1. */
        if (true == enable)
        {
    ...
    else if ((vector >= BSP_INT_SRC_AL0_SCI11_TEI11) && (vector <= BSP_INT_SRC_AL0_RSPI0_SPEI0))
    {
        /* Group AL0. */
        if (true == enable)
        {
